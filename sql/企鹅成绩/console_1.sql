...
where not exists(select 1
                 from t_score t
                 where
                    ss.exam_id = t.exam_id
                   and ss.subject_id = t.subject_id
                   and ss.student_id = t.student_id)
    and ss.exam_id = 127 and ss.subject_id != 0;
...
where ss.subject_id = 0
  ...
  and not exists(
        select 1
        from t_score t
        where ss.exam_id = t.exam_id
          and ss.student_id = t.student_id);

select * from t_examination where id = 146;
select * from t_examination_subject where exam_id = 146;
select * from t_examination_class where exam_id = 146;
select * from t_score where exam_id = 146;
select * from t_score_student_stat where exam_id = 146;
select * from t_score_class_stat where exam_id = 146;
select * from t_score_grade_stat where exam_id = 146;
select * from t_compositive_class_stat where exam_id = 146;
select * from t_compositive_grade_stat where exam_id = 146;
